      ...
      <div class="bg-white rounded-xl shadow-lg p-6 mb-8 border border-gray-200">
        <div class="flex flex-wrap gap-4 items-center">
          <div class="flex-1 min-w-48">
            ...
            <select v-model="filtroTipoEntidad" @change="filterByTipoEntidad" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500">
          ...
          <div class="flex-1 min-w-48">
            ...
            <select v-model="filtroTipoArchivo" @change="filterByTipoArchivo" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gradient-cfag">
              <tr>
                <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">Archivo</th>
                <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">Entidad</th>
                ...
                <th class="px-6 py-4 text-center text-xs font-bold text-white uppercase tracking-wider">Tamaño</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="documento in filteredDocumentos" :key="documento.id" class="hover:bg-gray-50 transition-colors">
                <td class="px-6 py-4">
                  <div class="flex items-center">
                    <div class="flex-shrink-0">
                      <svg v-if="getFileIcon(documento.tipo_archivo)" :class="getFileIconColor(documento.tipo_archivo)" class="h-8 w-8" fill="currentColor" viewBox="0 0 24 24">
                        <path v-html="getFileIcon(documento.tipo_archivo)"></path>
                      </svg>
                    </div>
                    <div class="ml-3">
                      <div class="text-sm font-bold text-gray-900">{{ documento.nombre_archivo }}</div>
                ...
                <td class="px-6 py-4">
                  <div class="text-sm text-gray-900">{{ documento.descripcion || 'Sin descripción' }}</div>
                </td>
                <td class="px-6 py-4 text-center whitespace-nowrap">
                  <span class="text-sm text-gray-600">{{ formatFileSize(documento.tamano_archivo) }}</span>
                </td>
                <td class="px-6 py-4 text-center whitespace-nowrap">
                ...
                <td class="px-6 py-4 text-center whitespace-nowrap">
                  <div class="flex items-center justify-center space-x-2">
                    <button @click="downloadDocument(documento)" class="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition-colors" title="Descargar">
                      <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                      </svg>
                    </button>
                    <button @click="viewDocument(documento)" class="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200 transition-colors" title="Ver">
                      <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                      </svg>
                    </button>
                    <button @click="confirmDelete(documento)" class="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-colors" title="Eliminar">
                      <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div>
            ...
            <div v-if="selectedFile" class="mt-3 p-3 bg-green-50 rounded-lg">
              <div class="flex items-center">
                ...
                <span class="text-xs text-green-600 ml-2">({{ formatFileSize(selectedFile.size) }})</span>
              </div>
            </div>
          </div>
