          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gradient-cfag">
              <tr>
                ...
                <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">Renglón Origen</th>
                <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">Renglón Destino</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="intra in intras" :key="intra.id" class="hover:bg-gray-50 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="text-sm font-bold text-gray-900">{{ formatDate(intra.fecha_transferencia) }}</span>
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 text-center whitespace-nowrap">
                  <span class="text-lg font-black text-blue-600">Q{{ formatMoney(intra.monto) }}</span>
                </td>
                <td class="px-6 py-4 text-center whitespace-nowrap">
                  <span :class="[
                ...
                <td class="px-6 py-4 text-center whitespace-nowrap">
                  <div class="flex items-center justify-center space-x-2">
                    <button v-if="intra.estado === 1" @click="confirmAnular(intra)" class="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-colors" title="Anular">
                      <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <form @submit.prevent="submitForm" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select
                v-model="form.renglon_origen_id"
                required
                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500"
              >
            ...
            <div>
              ...
              <select
                v-model="form.renglon_destino_id"
                required
                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500"
              >
                <option value="">Seleccionar renglón destino</option>
                <option v-for="renglon in renglones" :key="renglon.id" :value="renglon.id" :disabled="renglon.id == form.renglon_origen_id">
                  {{ renglon.codigo }} - {{ renglon.nombre }}
                </option>
              </select>
            </div>
