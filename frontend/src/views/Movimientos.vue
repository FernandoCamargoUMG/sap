<template>
  <AppLayout>
      ...
      <div class="bg-white rounded-xl shadow-lg p-6 mb-8 border border-gray-200">
        <div class="flex flex-wrap gap-4 items-center">
          <div class="flex-1 min-w-48">
            ...
            <select v-model="filtroTipo" @change="filterByTipo" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500">
              <option value="">Todos los tipos</option>
              <option v-for="tipo in tiposMovimiento" :key="tipo.id" :value="tipo.id">{{ tipo.nombre }}</option>
            </select>
          </div>
          <div class="flex-1 min-w-48">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gradient-cfag">
              <tr>
                ...
                <th class="px-6 py-4 text-center text-xs font-bold text-white uppercase tracking-wider">Monto Total</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="movimiento in filteredMovimientos" :key="movimiento.id" class="hover:bg-gray-50 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="text-sm font-bold text-gray-900">{{ formatDate(movimiento.fecha_movimiento) }}</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <span :class="[
                ...
                <td class="px-6 py-4 text-center whitespace-nowrap">
                  <div class="flex items-center justify-center space-x-2">
                    <button @click="viewDetails(movimiento)" class="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200 transition-colors" title="Ver detalles">
                      <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                      </svg>
                    </button>
                    <button v-if="movimiento.estado === 1" @click="confirmAnular(movimiento)" class="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-colors" title="Anular">
                      <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <form @submit.prevent="submitForm" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                v-model="form.documento_respaldo"
                type="text"
                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <div v-if="detalle.saldoDisponible !== null" class="text-xs mt-1">
                      <span :class="detalle.saldoDisponible >= detalle.monto ? 'text-green-600' : 'text-red-600'">
          ...
          <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
            ...
            <button
              type="submit"
              :disabled="submitting || form.detalles.length === 0 || !canSubmit"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-gray-50 rounded-xl">
            ...
            <div>
              ...
              <p :class="[
                ...
                {{ getMontoPrefix(selectedMovimiento.tipo) }}Q{{ formatMoney(selectedMovimiento.monto_total) }}
              </p>
            </div>
          </div>
              ...
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                  <tr>
                    ...
                    <th class="px-6 py-3 text-right text-xs font-bold text-gray-500 uppercase">Efecto</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                  <tr v-for="detalle in selectedMovimiento.detalles" :key="detalle.id" class="hover:bg-gray-50">
    ...
    <div v-if="showAnularModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
