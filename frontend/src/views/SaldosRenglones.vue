    ...
    <div class="p-6">
        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <div class="flex space-x-3">
                ...
                <button @click="loadRenglones" :disabled="loading"
        ...
        <div class="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input v-model="filtros.busqueda" type="text" placeholder="Código o nombre..."
                        ...
                        <tr v-for="renglon in renglonesFiltrados" :key="renglon.id" class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-sm text-gray-600">{{ renglon.porcentaje_ejecutado.toFixed(1)
                                        }}%</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <button @click="verDetalle(renglon.id)" class="text-blue-600 hover:text-blue-900 mr-3">
                                    Ver detalle
                                </button>
                                <button @click="registrarEjecucion(renglon.id)"
                ...
                <div class="mt-3">
                    ...
                    <div v-else-if="modalDetalle.data" class="space-y-6">
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    ...
                                    <span class="font-medium ml-2">Q {{
                                        formatMoney(modalDetalle.data.resumen.monto_inicial) }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Saldo disponible:</span>
                                    <span class="font-medium ml-2">Q {{
                                        formatMoney(modalDetalle.data.resumen.saldo_disponible) }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Total asignado:</span>
                                    <span class="font-medium ml-2">Q {{
                                        formatMoney(modalDetalle.data.resumen.monto_asignado) }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Por ejecutar:</span>
                                    <span class="font-medium ml-2">Q {{
                                        formatMoney(modalDetalle.data.resumen.saldo_por_ejecutar) }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="modalDetalle.data.presupuestos.length > 0">
                            ...
                            <div class="space-y-2">
                                <div v-for="presupuesto in modalDetalle.data.presupuestos"
                                    :key="presupuesto.presupuesto_id" class="bg-white p-3 border rounded-lg">
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <div class="text-sm font-medium">
                                                Presupuesto {{ presupuesto.presupuesto_anio }}/{{
                                                    String(presupuesto.presupuesto_mes).padStart(2, '0') }}
                                            </div>
                                            <div class="text-xs text-gray-500">ID: {{ presupuesto.presupuesto_id }}
                                            </div>
                                        </div>
                                        <div class="text-right">
                                            <div class="text-sm">Q {{ formatMoney(presupuesto.monto_asignado) }}</div>
                                            <div class="text-xs text-gray-500">{{ presupuesto.porcentaje_ejecucion }}%
                                                ejecutado</div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="mt-3">
                    ...
                    <form v-else @submit.prevent="procesarEjecucion" class="space-y-4">
                        <div v-if="modalEjecucion.presupuestosDisponibles.length > 1">
                            ...
                            <select v-model="modalEjecucion.presupuestoSeleccionado" @change="onPresupuestoChange"
                                required
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <option value="">Seleccionar presupuesto...</option>
                                <option v-for="presupuesto in modalEjecucion.presupuestosDisponibles"
                                    :key="presupuesto.id" :value="presupuesto.id">
                                    {{ presupuesto.presupuesto_anio }}/{{
                                        String(presupuesto.presupuesto_mes).padStart(2, '0') }} -
                        ...
                        <div v-else-if="modalEjecucion.presupuestosDisponibles.length === 1"
                            class="bg-blue-50 p-3 rounded-lg">
                            <p class="text-sm text-blue-800">
                                <strong>Presupuesto:</strong>
                                {{ modalEjecucion.presupuestosDisponibles[0].presupuesto_anio }}/{{
                                    String(modalEjecucion.presupuestosDisponibles[0].presupuesto_mes).padStart(2, '0') }}
                            </p>
                            <p class="text-sm text-blue-600">
                                Saldo disponible: Q {{
                                    formatMoney(modalEjecucion.presupuestosDisponibles[0].saldo_por_ejecutar) }}
                            </p>
                        </div>

                        <div v-if="modalEjecucion.presupuestosDisponibles.length === 0"
                            class="bg-yellow-50 p-3 rounded-lg">
                            <p class="text-sm text-yellow-800">No hay presupuestos disponibles para ejecutar en este
                                renglón.</p>
                        </div>

                        <div v-if="modalEjecucion.maxMonto > 0">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Monto a ejecutar</label>
                            <input v-model.number="modalEjecucion.monto" type="number" step="0.01" min="0.01"
                                :max="modalEjecucion.maxMonto" required
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <div>
                            ...
                            <textarea v-model="modalEjecucion.descripcion" rows="3"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                placeholder="Describe el concepto de la ejecución..."></textarea>
                        </div>

                        <div class="flex justify-end space-x-3">
                            <button type="button" @click="cerrarModalEjecucion"
                                class="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-400">
                                Cancelar
                            </button>
                            <button v-if="modalEjecucion.presupuestosDisponibles.length > 0" type="submit"
