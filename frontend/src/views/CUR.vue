      ...
      <div class="bg-white rounded-xl shadow-lg p-6 mb-8 border border-gray-200">
        <div class="flex flex-wrap gap-4 items-center">
          ...
          <div class="flex-1 min-w-48">
            ...
            <select v-model="filtroRenglon" @change="filterByRenglon" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500">
              <option value="">Todos los renglones</option>
              <option v-for="renglon in renglones" :key="renglon.id" :value="renglon.id">{{ renglon.codigo }} - {{ renglon.nombre }}</option>
            </select>
          </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gradient-cfag">
              <tr>
                ...
                <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">Renglón</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="cur in filteredCurs" :key="cur.id" class="hover:bg-gray-50 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="text-sm font-bold text-gray-900">{{ formatDate(cur.fecha_compromiso) }}</span>
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 text-center whitespace-nowrap">
                  <span class="text-lg font-black text-orange-600">Q{{ formatMoney(cur.monto_comprometido) }}</span>
                </td>
                <td class="px-6 py-4 text-center whitespace-nowrap">
                  <span :class="[
                ...
                <td class="px-6 py-4 text-center whitespace-nowrap">
                  <div class="flex items-center justify-center space-x-2">
                    <button @click="viewDetails(cur)" class="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200 transition-colors" title="Ver detalles">
                      <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                      </svg>
                    </button>
                    <button v-if="cur.estado === 1" @click="confirmAnular(cur)" class="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-colors" title="Anular">
                      <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-white rounded-2xl p-8 max-w-2xl w-full shadow-2xl">
        ...
        <form @submit.prevent="submitForm" class="space-y-6">
          ...
          <div>
            ...
            <input
              v-model="form.monto_comprometido"
              type="number"
              step="0.01"
              min="0"
              required
