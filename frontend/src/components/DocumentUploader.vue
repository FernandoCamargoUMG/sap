        ...
        <div class="bg-gray-50 rounded-xl p-6 border border-gray-200">
            ...
            <div v-if="selectedFiles.length > 0 && documentableId" class="mt-4">
                <button @click="uploadFile" :disabled="loading"
                    ...
                    <span>{{ loading ? 'Subiendo...' : `Subir ${selectedFiles.length} documento${selectedFiles.length >
                        1 ? 's' : ''}` }}</span>
                </button>
            </div>
                ...
                <div v-for="documento in documentos" :key="documento.id"
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p v-if="documento.descripcion" class="text-sm text-gray-600 mt-1">{{ documento.descripcion
                                }}</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-2">
                        <button @click="downloadDocument(documento)"
                            class="p-2 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors" title="Descargar">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 10v6m0 0l-4-4m4 4l4-4m5-2v8a2 2 0 01-2 2H5a2 2 0 01-2-2v-8a2 2 0 012-2h2.5" />
                            </svg>
                        </button>
                        <button @click="deleteDocument(documento)"
