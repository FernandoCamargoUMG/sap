        ...
        <div class="bg-gradient-to-br from-orange-500 to-orange-700 rounded-xl p-6 text-white">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-orange-200 text-xs">{{ (100 - resumen.porcentaje_ejecutado).toFixed(1) }}% disponible</p>
            </div>
            <div class="bg-white/20 rounded-lg p-2">
              <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-black">
                {{ resumen.total_variacion >= 0 ? '+' : '-' }}Q{{ formatMoney(Math.abs(resumen.total_variacion)) }}
              </p>
      ...
      <div class="bg-gray-100 rounded-xl p-4">
        ...
        <div class="w-full bg-gray-300 rounded-full h-4">
          <div 
            ...
            :style="`width: ${Math.min(resumen.porcentaje_ejecutado, 100)}%`"
          ></div>
        </div>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Renglón</th>
              <th class="px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Presupuestado</th>
              <th class="px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Ejecutado</th>
              <th class="px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Pendiente</th>
              <th class="px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">% Ejecución</th>
              <th class="px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Progreso</th>
            </tr>
          </thead>
            ...
            <tr v-for="renglon in renglones" :key="renglon.id" class="hover:bg-gray-50 transition-colors">
              <td class="px-6 py-4">
                <div class="flex items-center">
                  <div class="w-2 h-2 rounded-full mr-3" :class="getColorClass(renglon.porcentaje_ejecutado)"></div>
                  <div>
              ...
              <td class="px-6 py-4 text-center">
                <span class="text-sm font-bold text-blue-600">Q{{ formatMoney(renglon.monto_presupuestado) }}</span>
              </td>
              <td class="px-6 py-4 text-center">
                <span class="text-sm font-bold text-green-600">Q{{ formatMoney(renglon.monto_ejecutado) }}</span>
              </td>
              <td class="px-6 py-4 text-center">
                <span class="text-sm font-bold text-orange-600">Q{{ formatMoney(renglon.monto_pendiente) }}</span>
              </td>
              <td class="px-6 py-4 text-center">
                <span :class="[
                  'text-sm font-bold',
      ...
      <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
        ...
        <div class="space-y-4">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-bold text-green-600">{{ renglones?.filter(r => r.porcentaje_ejecutado > 0).length || 0 }}</span>
          </div>
          <div class="flex justify-between">
